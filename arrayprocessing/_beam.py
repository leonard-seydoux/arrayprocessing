...
from arrayprocessing.synthetic import cylindrical
...
class Beam():
    ...
    def __init__(self, antenna, frequency=None, slowness_max=None,
                 dimension=200, flip=False):
        ...
        if flip:
            angular_frequency *= -1
        phase_x = np.outer(self._slowness_grid[0].ravel(), antenna.x)
        phase_y = np.outer(self._slowness_grid[1].ravel(), antenna.y)
